...
if [ $PAIRED -eq 1 ]; then
	...
	if [ $STAGE -le 2 ]; then
		...
		if [ $GI -eq 1 ]; then
			...
			bsub -n $st_threads -J job4 -w "done(job2)" -o $OUT_DIR/star/${RUN_NAME}_STAR_euler_log.txt -e $OUT_DIR/star/${RUN_NAME}_STAR_log.txt STAR --runThreadN $st_threads --genomeDir $genomeDir --readFilesIn $OUT_DIR/trimmomatic/${RUN_NAME}_trimmed_1P.fastq $OUT_DIR/trimmomatic/${RUN_NAME}_trimmed_2P.fastq --outFileNamePrefix $OUT_DIR/star/$RUN_NAME --outFilterIntronMotifs $outFilterIntronMotifs --outSAMstrandField $outSAMstrandField
		else
...
# Conversion ran through in the first iteration, we have to repeat sorting.
#		# SAMTOOLS Conversion
...
#
		...
		bsub -n $sa_threads -J job6 -w "done(job4)" -o $OUT_DIR/samtools/${RUN_NAME}_STsort_euler_log.txt -e $OUT_DIR/samtools/${RUN_NAME}_STsort_log.txt samtools sort -@ $sa_threads -m 1G -o $OUT_DIR/samtools/${RUN_NAME}Aligned.out_sorted.bam $OUT_DIR/samtools/${RUN_NAME}Aligned.out.bam
#		bsub -n $sa_threads -J job6 -w "done(job4)" -o $OUT_DIR/samtools/${RUN_NAME}_STsort_euler_log.txt -e $OUT_DIR/samtools/${RUN_NAME}_STsort_log.txt samtools sort -@ $sa_threads -m 1G -O bam -T tmp $OUT_DIR/samtools/${RUN_NAME}Aligned.out.bam > $OUT_DIR/samtools/${RUN_NAME}Aligned.out_sorted.bam
	...
	if [ $STAGE -le 2 ]; then
		...
		if [ ! -s ${genomeDir}/Genome ]; then
			...
			GI=0
			echo "No indexed Genome found in '$genomeDir'. Creating one from scratch!"
			bsub -n $st_threads -R "rusage[mem=4096, scratch=1000]" -J job3 -o $genomeDir/STAR_gi_euler_log.txt -e $genomeDir/STAR_gi_log.txt STAR --runThreadN $st_threads --runMode genomeGenerate --genomeDir $genomeDir --genomeFastaFiles $genomeFastaFiles --sjdbGTFfile $sjdbGTFfile --sjdbOverhang $sjdbOverhang
		else
			GI=1
			echo "Found indexed genome in '$genomeDir'. Starting to map reads with Star..."
		fi
